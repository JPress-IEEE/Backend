...
const handleVideoCallSockets = (io: Server) => {
  io.on("connection", (socket: Socket) => {
    ...
    socket.on("requestCall", async (chatId: string, senderId: string, receiverId: string, callback) => {
      try {
        ...
        io.to(receiverId).emit("incomingCall", { call, from: senderId });
    ...
    socket.on("acceptCall", async (chatId: string, receiverId: string, senderId: string, callback) => {
      try {
        ...
        io.to(chatId).emit("callAccepted", { call });
    ...
    socket.on("declineCall", async (chatId: string, receiverId: string, senderId: string, callback) => {
      try {
        ...
        io.to(chatId).emit("callDeclined", { call });
    ...
    socket.on("endCall", async (chatId: string, senderId: string, receiverId: string, callback) => {
      try {
